...
class hardQuestions():
    ...
    def get_hovered_option(self):
        ...
        if self.current_question_index < len(self.hardquestions):
            ...
            self.hovered_option = self.hardquestions[self.current_question_index]["correct_answer"]
    ...
    def reset_game(self):
        ...
        self.player.x = 200
        self.enemy.x = 10
        random.shuffle(self.hardquestions) 
    def finish(self):
        if self.player.x > 1280:
            screen.blit(screen, (0,0))
            pygame.display.update()
            ...
            # Check if you have more questions to display
            if self.current_question_index < len(self.hardquestions):
                self.display_question(self.hardquestions[self.current_question_index], self.hovered_option)

    def get_clicked_option(self):
        mouse_x, mouse_y = pygame.mouse.get_pos()
                
        if self.current_question_index < len(self.hardquestions):
            correct_answer = self.hardquestions[self.current_question_index]["correct_answer"]
            if 150 <= mouse_y <= 200:
                global chosen_option
                chosen_option = self.hardquestions[self.current_question_index]["options"][0]
            elif 200 <= mouse_y <= 250:
                chosen_option = self.hardquestions[self.current_question_index]["options"][1]
            elif 250 <= mouse_y <= 300:
                chosen_option = self.hardquestions[self.current_question_index]["options"][2]
            elif 250 <= mouse_y <= 350:
                chosen_option = self.hardquestions[self.current_question_index]["options"][3]
            else:
                chosen_option = None
